...
############## My parser
parser.add_argument('--small_epsilon', type=float, default=config.small_epsilon, help='the epsilon for small perturbation pixels')
parser.add_argument('--sig_beta', type=float, default=config.sig_beta, help='the beta for significant perturbation pixels')
...
parser.add_argument('--search_region', type=str2bool, default=config.search_region,  help='flag specifying where to search the significant perturbed region or not')
############ End of my parser
...
######### My coding
if(config.sig_beta!=0 and config.search_region==False):
    assert args.left_bottom_corner!= None and args.right_up_corner != None, "two points of the region must be provided"
    assert args.left_bottom_corner[0] >=0 and args.left_bottom_corner[1] >=0, "The coordinates of the left_bottom_corner CANNOT be less than 0"
    x_min = args.left_bottom_corner[0]
    x_max = args.right_up_corner[0]
    y_min = args.left_bottom_corner[1]
    y_max = args.right_up_corner[1]
    if(dataset=='mnist'):
        assert x_max <=27 and y_max <=27, "The coordinates of the right_up_corner CANNOT be greater than the height and width of the MNIST image"
        region_matrix = np.zeros((28,28),dtype=int)
        height = y_max - y_min + 1
        width = x_max - x_min + 1
        sig_area_matrix = np.ones((height, width), dtype=int)
        region_matrix[y_min:y_max+1, x_min:x_max+1] = sig_area_matrix
    elif(dataset=='cifar10'):
        assert x_max <=31 and y_max <=31, "The coordinates of the right_up_corner CANNOT be greater than the height and width of the CIFAR10 image"
        region_matrix = np.zeros((32,32),dtype=int)
        height = y_max - y_min + 1
        width = x_max - x_min + 1
        sig_area_matrix = np.ones((height, width), dtype=int)
        region_matrix[y_min:y_max+1, x_min:x_max+1] = sig_area_matrix
    else:
        assert dataset in ['mnist', 'cifar10'], "only mnist, cifar10 datasets are supported currently"
########### End of my coding
...
if dataset=='acasxu':
	...
	#NOT. Enter this branch if user specifies zonotope matrix using --zonotope argument
	...
	#NOT. Enter this branch if user specifies --geometric to indicate geometric analysis
    ...
    baseline_list = [3, 13, 17, 25, 28, 48, 54, 56, 60, 69, 70, 71, 79, 82, 85, 86, 88, 99]
    total_time = 0
    success_list = []
    if(config.search_region == False):
        for i, test in enumerate(tests):
            if config.from_test and i < config.from_test:
                continue
            ...
            image= np.float64(test[1:len(test)])/np.float64(255) #test from index 1 is the sequence of the image's pixel intensities
            # prepocess the image such that the intensity is within the range of [0,1]
            ...
            if(label == int(test[0])): #test[0] is the ground truth lable
                # Only check the robustness against the image whose original version is correctly classified by the network.
                perturbed_label = None

                specLB = np.clip(image - epsilon,0,1)
                specUB = np.clip(image + epsilon,0,1)
                # In our setting, we would have a matrix indicating sig\small pb area. With 1 meaning sig and 0 meaning small
                # Also have the epsilon for small pb and beta for sig pb
                ################# Coding of my part
                if(config.sig_beta!=0):
                    my_epsilon = config.small_epsilon
                    my_beta = config.sig_beta
                    flatten_maxtrix = region_matrix.flatten()
                    sig_indexes = np.where(flatten_maxtrix == 1)
                    small_indexes = np.where(flatten_maxtrix == 0)
                    if(dataset=='mnist'):
                        # the region_matrix is np.array()
                        specLB[sig_indexes] = np.clip(image - my_beta,0,1)[sig_indexes]
                        specLB[small_indexes] = np.clip(image - my_epsilon,0,1)[small_indexes]
                        specUB[sig_indexes] = np.clip(image + my_beta,0,1)[sig_indexes]
                        specUB[small_indexes] = np.clip(image + my_epsilon,0,1)[small_indexes]
                    elif(dataset=='cifar10'):
                        for index in sig_indexes:
                            count=3*index
                            specLB[count] = max(min(1, image[count]-my_beta), 0)
                            specUB[count] = max(min(1, image[count]+my_beta), 0)
                            count = count + 1
                            specLB[count] = max(min(1, image[count]-my_beta), 0)
                            specUB[count] = max(min(1, image[count]+my_beta), 0)
                            count = count + 1
                            specLB[count] = max(min(1, image[count]-my_beta), 0)
                            specUB[count] = max(min(1, image[count]+my_beta), 0)
                        for index in small_indexes:
                            count=3*index
                            specLB[count] = max(min(1, image[count]-my_epsilon), 0)
                            specUB[count] = max(min(1, image[count]+my_epsilon), 0)
                            count = count + 1
                            specLB[count] = max(min(1, image[count]-my_epsilon), 0)
                            specUB[count] = max(min(1, image[count]+my_epsilon), 0)
                            count = count + 1
                ...
                per_img_time = end-start
                total_time += per_img_time
                print(per_img_time, "seconds")
            else:
                print("img",i,"not considered, correct_label", int(test[0]), "classified label ", label)

        print('analysis precision ',verified_images,'/ ', correctly_classified_images)
        print (success_list)
        print ("Total analysis time:", total_time, "seconds")
    else:
        #If the search-region flag is True
        iteration_count = 0
        tests_in_list = list(tests)
        #for line in tests:
        #ss	print (lines)
        success_flag_finding_region = 0
        for loop_count in range(1000):
            #print (loop_count)
            # Random generate the region
            success_list = []
            x_min = randrange(26)
            y_min = randrange(26)
            x_max = x_min + 2
            ...
            sig_area_matrix = np.ones((height, width), dtype=int)
            region_matrix[y_min:y_max + 1, x_min:x_max + 1] = sig_area_matrix
            # End of rasssndom generation of the region
            for i in baseline_list:
            	# i is the image id
                #print ('potential img', i)
                iteration_count += 1
                test = tests_in_list[i]
                image = np.float64(test[1:len(test)]) / np.float64(255)  # test from index 1 is the sequence of the image's pixel intensities
                perturbed_label = None
                specLB = np.clip(image - epsilon, 0, 1)
                specUB = np.clip(image + epsilon, 0, 1)
                ...
                if (config.sig_beta != 0):
                    ...
                    if (dataset == 'mnist'):
                        ...
                        assert dataset in ['mnist',
                                           'cifar10'], "only mnist, cifar10 datasets are supported currently"
                ################# End of my coding
                normalize(specLB, means, stds, dataset)
                normalize(specUB, means, stds, dataset)
                start = time.time()
                perturbed_label, _, nlb, nub = eran.analyze_box(specLB, specUB, domain, config.timeout_lp,
                                                                config.timeout_milp, config.use_default_heuristic)
                #print("nlb ", nlb[len(nlb) - 1], " nub ", nub[len(nub) - 1])
                if (perturbed_label == int(test[0])):
                    print("img", i, "Verified", perturbed_label)
                    success_list.append(i)
                else:
                    print("img", i, "Failed")
                    break
                ...
                per_img_time = end - start
                total_time += per_img_time
            if len(success_list)==len(baseline_list):
                success_flag_finding_region = 1
                print ("left bottom corner of the region:(", x_min, y_min, ").")
                break
            else:
                print ("This region is not qualified!")
                continue
        if success_flag_finding_region == 0:
            print ("Fail to find the region within 1000 trials!")
        print ("Total analysis time:", total_time, "seconds")
        print ("Iteration number is", iteration_count)